...
def redact_exception(exc: BaseException, dsn: str) -> str:
    return _CUTOVER.redact_exception(exc, dsn)
...
def resolve_restore_dsn(explicit: str | None) -> str:
    restore = (explicit or os.environ.get("PHIGRAPH_G14_RESTORE_DSN", "")).strip()
    if not restore:
        print("restore DSN is required via --restore-dsn or PHIGRAPH_G14_RESTORE_DSN", file=sys.stderr)
        raise SystemExit(EXIT_PRECONDITION)
    return restore
...
def validate_backup_bytes(backup_path: Path) -> None:
    ...
    if size_bytes <= 0:
        ...
        raise SystemExit(EXIT_PRECONDITION)
    if not raw.startswith(PGDMP_MAGIC):
...
def verify_manifest_and_backup(manifest: dict[str, Any], *, dsn: str) -> dict[str, Any]:
    validate_manifest_structure(manifest)
    backup_path = Path(str(manifest["backup_path"])).resolve()
    validate_backup_bytes(backup_path)
...
def run_backup(*, source_dsn: str, artifact_dir: Path, run_id: str | None) -> dict[str, Any]:
    ...
    backup_path = artifact_dir / f"g14_{resolved_run_id}.dump"
    manifest_path = artifact_dir / f"g14_{resolved_run_id}.manifest.json"
...
def run_verify_manifest(*, manifest_path: Path, source_dsn: str, expect_failure: bool = False) -> dict[str, Any]:
    ...
    try:
        verification = verify_manifest_and_backup(manifest, dsn=source_dsn)
...
def run_restore(
    *,
    ...
    resolved_restore_dsn = restore_dsn or resolve_restore_dsn(None)
    ...
    try:
        ...
        run_pg_restore(resolved_restore_dsn, Path(str(manifest["backup_path"])))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    restore.add_argument("--restore-dsn")
    ...
    drill.add_argument("--restore-dsn", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "backup":
            ...
            restore_dsn = args.restore_dsn or resolve_restore_dsn(None)
            dsns = (source_dsn, restore_dsn)
            report = run_restore(
                manifest_path=args.manifest,
                source_dsn=source_dsn,
                restore_dsn=restore_dsn,
        ...
        print(redact_exception(exc, source_dsn), file=sys.stderr)
        if report is None:
            report = {
                "tool_version": TOOL_VERSION,
