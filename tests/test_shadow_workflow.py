import pandas as pd
from phigraph.shadow_workflow import ShadowWorkflowConfig, run_shadow_deployment_workflow

def test_shadow_workflow(tmp_path):
    tables = {
        "fuel": pd.DataFrame({
            "camion":[f"KLG-{100+i%8}" for i in range(40)],
            "surtidor":[f"S{i%3}" for i in range(40)],
            "litros":[400+i for i in range(40)],
        }),
        "trips": pd.DataFrame({
            "equipo":[str(100+i%8) for i in range(40)],
            "ruta":[f"R{i%4}" for i in range(40)],
            "toneladas":[100+i%10 for i in range(40)],
        }),
    }
    report = run_shadow_deployment_workflow(
        tables,
        ShadowWorkflowConfig(
            n_null_controls=5,
            human_approval=True,
            rollback_available=True,
            operations_score=.8,
            shadow_store_path=str(tmp_path/"shadow.json"),
            decision_audit_path=str(tmp_path/"audit.json"),
        ),
        reference_tables=tables,
    )
    assert report["results"][-1]["agent"] == "shadow_deployment"
    assert report["artifacts"]["shadow_case"]["executed"] is False
