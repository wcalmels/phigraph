...
5. **`append_scoped_once` authority** — PostgreSQL `PRIMARY KEY (tenant_id,
   project_id, collection, canonical_key)` is authoritative. On conflict the
   engine reads the existing row and compares `payload_hash`. Same hash →
   `created=False`; different hash → `DuplicateCanonicalKey`. No blind
